...
まず$b = 0$よりZardusはアダマールゲートを作用させることなく基底$\{\ket{0}, \ket{1}\}$で測定を行う。すでに述べたようにこの2量子ビットは測定すると$\ket{00}$か$\ket{11}$がそれぞれ$\frac{1}{2}$で観測されるため、Zardusの結果$z$によって次のように分岐する。
もし$z = 0$ならば、このときHackerの持つ量子ビットは$\ket{0}_2$に収束するため、回転をせずに基底$\{\ket{0}, \ket{1}\}$で測定した場合は100%の確率で$h = 0$となる。しかしHackerは$b = 0$であることを知らないため、$z = 0$であるなら$h = 0$でよいが一方で$z = 1$であるなら$h = 1$でなければ勝利しない。このままだとランダムで$h$を決めることになってしまい勝率は$\frac{1}{2}$となってしまう。ここで`rotate left`をしてから$\{\ket{0}, \ket{1}\}$基底で測定するとどうなるかを考える。
