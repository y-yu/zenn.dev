...
$$
\frac{1}{\sqrt{2}}\left(\ket{0}_1\ket{0}_2 + \ket{1}_1\ket{1}_2\right) = CX\ket{+}_1\ket{0}_1
$$
...
さて問題ではCompetitor Aのベットを知ったうえで、`magic qoin`を左右に回転（_rotate_）させるか、あるいは何もしないかを選ぶことができる。「左右」のような空間的な表現が登場したところで、説明を直感的にするために**ブロッホ球**という量子ビットの空間的な表現を導入する。

![](https://storage.googleapis.com/zenn-user-upload/6163240a0b717ebf9ed3029f.png =500x)

このように$x, y, z$の3つの軸があり、この1量子ビットは実はこの球の表面の座標に対応する。たとえばこれまで紹介してきた量子ビットはブロッホ球では次のような位置になる。

![](https://storage.googleapis.com/zenn-user-upload/6a4a6a6c4f4f04a9e19abc36.png =400x)


ここでの左右の回転とは何か？というと配布されているソースコード`coin.py`[^source_code_1]から次のような操作であることがわかる。
...
この行列表現からは分かりにくいかもしれないが、$R_y(\theta)\ket{a}$は「1量子ビット$\ket{a}$を$y$軸中心に$\theta$回転させる」ことを意味しているが

## Zardusの量子操作

ソースコード`players.py`[^source_code_2]からZardusはCompetitor Bの結果に応じて次の量子操作を行う。

- Competitor Bのベットが`0`であるとき
    - なにもしない
- Competitor Bのベットが`1`であるとき
    - 量子ビットにアダマールゲート$H$を作用させる
