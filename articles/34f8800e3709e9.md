...
最終的には筆者の実装もこれと同じテクニックを利用して高速化を達成するが、一方でこの`setInt`や`setString`のような低レベル[^low_level]な関数はたとえばインデックスを間違えたりテーブル定義上`int`なカラムに`setString`すればランタイムエラーになる可能性がある。またテーブル定義や`Book`のデータ構造が変化した場合には、このクエリー生成部分も修正しなければランタイムエラーとなる。これらのことからSlick標準の`++=`と比べて効率はよいが一方で保守性は低下したと言わざるを得ない。
